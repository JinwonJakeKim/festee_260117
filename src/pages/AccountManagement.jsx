...
import { manageAccountStatus } from "@/functions/manageAccountStatus";
...
export default function AccountManagement() {
  ...
  const handleAction = async () => {
    ...
    try {
      await manageAccountStatus({ action: activeAction, email_confirm: emailInput });
      setSuccess(true);
      // 3초 후 로그아웃 처리
      setTimeout(() => {
        base44.auth.logout("/");
      ...
      setError(e?.response?.data?.error || "오류가 발생했습니다. 다시 시도해주세요.");
    } finally {
      setIsLoading(false);
    }
  };
