          ...
          <TabsContent value="festivalExtraction" className="mt-4 space-y-4">
            ...
            <Tabs defaultValue="pending" className="w-full">
              ...
              <TabsContent value="pending" className="mt-4 space-y-3">
                ...
                {linksList.filter(r => r.processing_status === 'pending' && (!linkSearchQuery.trim() || r.url?.toLowerCase().includes(linkSearchQuery.toLowerCase()))).length > 0 ? (
                  linksList.filter(r => r.processing_status === 'pending' && (!linkSearchQuery.trim() || r.url?.toLowerCase().includes(linkSearchQuery.toLowerCase()))).map((item) => {
              ...
              <TabsContent value="processed" className="mt-4 space-y-3">
                  ...
                  <Card className="bg-gray-900 border-gray-800 p-4">
                    ...
                    {selectedLinkIds.size > 0 && (
                      <div className="flex gap-2">
                        ...
                        >
                          {batchExtractionProgress.isExtracting ? (
                            <><Loader2 className="w-4 h-4 mr-2 animate-spin" />재추출 중...</>
                          ) : (
                            <><RefreshCw className="w-4 h-4 mr-2" />재추출 ({selectedLinkIds.size}개)</>
                          )}
                        </Button>
                        <Button
                          onClick={handleDeleteSelectedLinks}
                ...
                {linksList.filter(r => r.processing_status === 'processed' && (!linkSearchQuery.trim() || r.url?.toLowerCase().includes(linkSearchQuery.toLowerCase()))).length > 0 ? (
                  linksList.filter(r => r.processing_status === 'processed' && (!linkSearchQuery.trim() || r.url?.toLowerCase().includes(linkSearchQuery.toLowerCase()))).map((item) => {
                    const rawData = rawDataList.find(r => r.id === item.raw_data_id);
                    return (
                    <Card key={item.id} className={`border-2 ${
                      selectedLinkIds.has(item.id) 
