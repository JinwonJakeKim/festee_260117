      ...
      extraction = await base44.integrations.Core.InvokeLLM({
        prompt: `
          ...
          **🔥 DOM에서 직접 추출한 정보 (반드시 사용!):**
          ...
          - restrictions_original: 웹페이지에 금지사항/주의사항이 **명확하게** 적혀있지 않으면 빈 배열([])로 두세요
          - recommendations_original: 웹페이지에 추천 복장/준비물이 **명확하게** 적혀있지 않으면 빈 배열([])로 두세요
          - **절대로 일반적이거나 그럴듯한 내용을 만들어내지 마세요!** 실제 웹페이지에 있는 내용만 추출하세요.
